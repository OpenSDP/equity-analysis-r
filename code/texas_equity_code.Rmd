...
In this guide, you will be able to use standard statistics and data visuals to
...
defined in the repository's `R` folder. Here is the usage information:
...
among all grades: 3-8. The number of top gaps shown can be changed based on user
inputs, as well as the gap categories and subject areas measured.
...
than 0.1 usually translates to large differences in effective classroom time.
...
deviations on these exams (stored in `sd.table`). If these standard deviations
...
a small sample size, an exaggerated gap could be due to noise in the data.
...
Ideally, both qualitative (observational) and quantitative assessment could be
done to determine if there are discriminatory practices at a certain school. The
gold standard of these analyses would be a randomized trial. Even if practices
are not determined to be strictly discriminatory, schools with large gaps could
potentially learn from the policies and practices of schools at which the gap is
much lower in magnitude. This visualization could provide impetus for the start
a collaborative conversation between campuses.
